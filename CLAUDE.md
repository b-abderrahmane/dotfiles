...
- **tmux status bar**: data comes from `tmux/statusbar.sh` (symlinked to `~/.config/tmux/statusbar.sh`), invoked per-segment from `status-right` as `#($HOME/.config/tmux/statusbar.sh <segment>)`. Segments: `battery`, `cpu` (load/cores), `ram` (used/total), `disk` (used/total), `ip`, `dns`. The script is OS-aware (macOS `pmset`/`scutil`/`ipconfig`, Linux `/sys`/`resolvectl`/`ip route`) and prints `—` rather than erroring when a source is unavailable. `tmux.conf` owns layout/colors (Catppuccin-Mocha-ish palette); the script owns data. Reload with `prefix + r`.
