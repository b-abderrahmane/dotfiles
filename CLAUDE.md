...
- **Prompt**: `zsh/zshrc` loads [Starship](https://starship.rs) (`command -v starship && eval "$(starship init zsh)"`) when it's installed, otherwise falls back to the oh-my-zsh `ZSH_THEME`. Install with `brew install starship` (add to `check.sh` if it should be a tracked prerequisite). The prompt shows directory, git branch/status, **kubernetes context** (only when one is set — `[kubernetes] disabled = false` in `starship/starship.toml`), command duration, and contextual language versions. Glyphs need a Nerd Font (already a `check.sh` requirement).
- **tmux status bar**: data comes from `tmux/statusbar.sh` (symlinked to `~/.config/tmux/statusbar.sh`), invoked per-segment from `status-right` as `#($HOME/.config/tmux/statusbar.sh <segment>)`. Segments: `battery`, `cpu` (load/cores), `ram` (used/total), `disk` (used/total), `net` (↓down ↑up throughput), `ip`, `dns`. The `net` segment is a *rate*, so it stashes the previous interface byte counters + timestamp in `$TMPDIR/tmux-statusbar-net-<iface>` and reports the delta over elapsed time — the first sample of a session shows `↓— ↑—` until there's a baseline. The script is OS-aware (macOS `pmset`/`scutil`/`ipconfig`, Linux `/sys`/`resolvectl`/`ip route`) and prints `—` rather than erroring when a source is unavailable. `tmux.conf` owns layout/colors (Catppuccin-Mocha-ish palette); the script owns data. Reload with `prefix + r`.
